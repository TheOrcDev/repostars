...
const hexToRgb = (hex: string): [number, number, number] => {
  ...
  return m
    ? [
        Number.parseInt(m[1], HEX_RADIX) / MAX_BYTE,
        Number.parseInt(m[2], HEX_RADIX) / MAX_BYTE,
        Number.parseInt(m[3], HEX_RADIX) / MAX_BYTE,
      ]
    : [1, 1, 1];
};
...
export function SideRays({
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      const loop = (t: number) => {
        ...
        try {
          ...
          // Stop animating if the GL context throws.
        }
      };
    ...
    initializeWebGL().catch(() => undefined);
